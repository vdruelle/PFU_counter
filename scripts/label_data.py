...
def predict_full_dataset(model_save_path, image_folder, output_label_folder, num_classes=4, show=False):
    """
    ...
    for ii, image_name in enumerate(image_list):
        ...
        idxs = utils.batch_cleanup_boxes(boxes, scores, labels, 0.2)

        if not show:
            image_width, image_height = image.shape[2], image.shape[1]
            df = pd.DataFrame(columns=["label", "x1", "y1", "x2", "y2"])
            df["label"] = labels[idxs].tolist()
            df[["x1", "y1", "x2", "y2"]] = boxes[idxs].tolist()
            ...
            utils.plot_plate_detector(image, output, threshold=0.2)
            plt.show()
