    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for images, targets in dataloader["valid"]:
                ...
                error = compute_validation_errors(outputs, targets)
...
def predict_plate():
    ...
    model.roi_heads.box_predictor = FastRCNNPredictor(in_features, num_classes)
    model.to(device)
    model.load_state_dict(torch.load("model_saves/Plate_detection.pt"))  # Loading the trained model

    model.eval()
    with torch.no_grad():
        # for images, targets in dataloader["valid"]:
        images, targets = iter(dataloader["valid"]).next()
        images = list(image.to(device) for image in images)
